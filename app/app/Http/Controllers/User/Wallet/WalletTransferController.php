...
class WalletTransferController extends Controller
{
    ...
    {   
        ...
        // Debugging: Check if route exists
        try {
            $redirectUrl = route('user.identity.verification');
        } catch (\Exception $e) {
            Log::error('Route not found', ['error' => $e->getMessage()]);
            $redirectUrl = url('user/identity-verification'); // Fallback URL
        }
        ...
        if (empty($user->bvn) && empty($user->nin)) {
            return response()->json([
                'status' => 'redirect',
                'message' => 'Please complete your KYC by adding either BVN or NIN.',
        ...
        // Correct logging - pass arrays, not objects
        Log::error('Transfer validated', ['user_id' => $user->id]);
        Log::error('Transfer bvn', ['bvn' => $user->bvn]);
        Log::error('Transfer nin', ['nin' => $user->nin]);
