...
<script>
    ...
    $(document).ready(function () {
        $('#create-recipient-form').on('submit', function (e) {
            ...
            // Step 1: Verify PIN
            $.ajax({
                url: "{{ route('user.wallet.verifyPin') }}", // Your route for PIN verification
                method: "POST",
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                },
                data: {
                    pin: $transactionPin.val()
                },
                success: function(pinResponse) {
                    if (pinResponse.success) {
                                ...
                                if (response.status === 'success') {
                                    ...
                                    $.ajax({
                                        ...
                                        data: JSON.stringify({
                                            ...
                                            amount: amount,
                                            reason: 'Payment', // Consider making this dynamic
                        ...
                        // PIN is incorrect
                        toastr.error(pinResponse.message || 'Invalid transaction PIN.', 'PIN Verification Failed');
                        resetButtonState($button);
                    }
                },
                error: function () {
