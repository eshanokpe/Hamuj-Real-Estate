                        ...
                        <span>
                            ...
                            @else
                                <div class="wallet-dropdown-container position-relative">
                                    <span class="wallet-balance-display d-flex align-items-center" id="activeBalanceDisplay" style="cursor: pointer; white-space: nowrap;">
                                        <span id="activeBalanceText" class="me-1">Loading...</span>
                                        <svg class="dropdown-indicator" width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1L5 5L9 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                    </span>
                                    <div class="wallet-options-dropdown shadow-sm" style="display: none; position: absolute; background-color: white; border: 1px solid #e0e0e0; border-radius: 0.25rem; z-index: 1050; min-width: 180px; padding: 0.5rem 0; right: 0; margin-top: 0.25rem;">
                                        @php
                                            $hasWalletData = isset($wallet);
                                            $primaryBalanceExists = $hasWalletData && property_exists($wallet, 'balance');
                                            $gbpBalanceExists = $hasWalletData && property_exists($wallet, 'gbp_balance');
                                            $usdBalanceExists = $hasWalletData && property_exists($wallet, 'usd_balance');
                                            $anyBalanceExists = $primaryBalanceExists || $gbpBalanceExists || $usdBalanceExists;
                                        @endphp

                                        @if($hasWalletData && $anyBalanceExists)
                                            @if($primaryBalanceExists)
                                                <a href="#" class="dropdown-item wallet-select-option px-3 py-2 d-block text-decoration-none text-dark"
                                                   data-currency-symbol="{{ $wallet->currency ?? '' }}"
                                                   data-balance-value="{{ $wallet->balance ?? 0.00 }}"
                                                   data-label="{{ $wallet->currency ?? 'Primary' }}">
                                                    {{ $wallet->currency ?? 'Primary' }}: {{ ($wallet->currency ?? '') . number_format($wallet->balance ?? 0.00, 2) }}
                                                </a>
                                            @endif
                                            @if($gbpBalanceExists)
                                                <a href="#" class="dropdown-item wallet-select-option px-3 py-2 d-block text-decoration-none text-dark"
                                                   data-currency-symbol="GBP "
                                                   data-balance-value="{{ $wallet->gbp_balance ?? 0.00 }}"
                                                   data-label="GBP">
                                                    GBP: {{ 'GBP ' . number_format($wallet->gbp_balance ?? 0.00, 2) }}
                                                </a>
                                            @endif
                                            @if($usdBalanceExists)
                                                <a href="#" class="dropdown-item wallet-select-option px-3 py-2 d-block text-decoration-none text-dark"
                                                   data-currency-symbol="USD "
                                                   data-balance-value="{{ $wallet->usd_balance ?? 0.00 }}"
                                                   data-label="USD">
                                                    USD: {{ 'USD ' . number_format($wallet->usd_balance ?? 0.00, 2) }}
                                                </a>
                                            @endif
                                        @elseif($hasWalletData)
                                            <span class="dropdown-item px-3 py-2 d-block text-muted">No balances available</span>
                                        @else
                                            <span class="dropdown-item px-3 py-2 d-block text-muted">Wallet not available</span>
                                        @endif
                                    </div>
                                </div>
